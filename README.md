...
自动识别平台并解析小红书、B站、抖音、微博的媒体链接，返回无水印图片与视频（视频直接发送到聊天，无需点链接）。群里有人发链接即可自动触发，无需命令。
...
- **B站**：标题、UP主、时长、清晰度、封面、视频**直接发送到聊天**（见下方「视频直发」）
- **抖音**：标题、作者、点赞/评论/分享、无水印视频**直接发送**；图集作品逐张发送原图（最多 9 张）
- **微博**：正文、作者、转发/评论/点赞数、配图（最多 9 张）、视频直接发送（如有）

> ⚠️ 请确保链接可公开访问；部分平台可能因反爬策略导致解析失败。下载链接仅供个人学习使用，请遵守平台规范。

#### 解析增强

- **链接自动解析（无需命令）**：群里有人发 小红书/B站/抖音/微博 的链接时，机器人自动解析并回复，不用再发 `/解析`。命令消息不会重复触发；同一条链接在同一个群里默认 30 分钟内只自动解析一次（防止反复转发刷屏），`/解析` 命令不受此限制可随时强制重解析；`/开关 off media` 可按群同时关闭命令与自动解析。
- **视频直发**：解析到视频时，机器人直接把视频发到聊天里，而不是甩一条点不开的防盗链直链。B站经 LeiZ API 由服务端下载合并好的 MP4（清晰度可在配置中下拉选择：480P / 720P / 1080P / 4K / 自动最高，默认 1080P）；抖音发送无水印视频。视频过大（超过大小上限）或下载失败时自动改为发送链接。临时文件落 `/tmp/astrbot_media`，1 小时自动清理。
- **LeiZ API 解析（B站/抖音）**：已配置 `leiz_api_key` 时，B站/抖音解析优先走 LeiZ API——B站可解锁游客拿不到的高清晰度（最高 4K，视站点账号权益与视频源而定），抖音拿无水印直链并支持图集；请求不直连平台官方接口，不受 B站风控波及。LeiZ 不可用时自动回退官方路径，功能不中断。
...
| 配置项 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `media_auto_parse_enable` | bool | true | 【自动解析】群里有人发支持平台的链接时，不用发命令、自动解析并回复 |
| `media_auto_parse_dedup_min` | int | 30 | 【自动解析】同一条链接在同一个群里多少分钟内只自动解析一次。0 = 每次都解析 |
| `media_video_send_enable` | bool | true | 【视频直发】解析到视频时直接把视频发到聊天里。发不出去时自动改为发链接 |
| `media_video_max_mb` | int | 100 | 【视频直发】视频最大发多大（MB）。超过就只发链接 |
| `media_video_quality` | string | 1080P | 【视频直发】B站视频清晰度（下拉：480P / 720P / 1080P / 4K / 自动最高）。需配置 `leiz_api_key`；4K 文件大，长视频可能只发链接 |
...
郊狼控制页可在服务器上**一键部署官方 v3 / v4 中转服务器**（[dglab-websocket-server](https://github.com/dungeonlab-open/dglab-websocket-server)，Bun 运行时），无需命令行操作：

- **自动检测**：v3（端口 9999）与 v4（端口 9998）各自独立检测部署状态；已部署的显示运行状态、systemd 服务名、源码版本，并支持一键卸载（历史手工部署的服务也能被识别接管）
- **一键部署（两步确认）**：点击后先完整披露将执行的系统级操作，再**键入版本号**（如 `v3`）确认才会执行（后端同样校验，绕过前端直接调用接口会被拒绝）；自动完成 安装 Bun → 克隆官方仓库 → 写配置 → 装依赖 → 创建 systemd 常驻服务（开机自启、崩溃自动拉起）→ 协议自检，全程约 10~60 秒
- **暴露公网开关**（默认关闭）：官方服务端监听地址固定为全接口，「本机/公网」可达性由防火墙控制——开关关闭时端口不放行（仅本机 `ws://127.0.0.1:端口` 可达，页面显示本机地址）；开启时自动放行端口（ufw）并探测显示公网地址 `ws://公网IP:端口`，关闭时自动收回。放行前有二次确认（接口同样要求确认标记）
- **卸载**：停止并删除 systemd 服务、收回防火墙放行（需二次确认）；源码目录保留，重新部署秒级完成

> 🔒 **安全披露（致安全审核与用户）**
> 「一键部署 / 卸载 / 暴露公网」是**面向服务器管理员的可选运维功能**，设计上满足以下约束：
> - **管理员鉴权**：所有 Page API 经 AstrBot 框架 `/api/v1/plugins/extensions/*` 路由访问，该路由由框架强制要求 WebUI 管理员凭证（`require_plugin_scope`），未登录管理员无法触达任何接口；普通聊天用户无任何途径触发。
> - **显式知情确认**：部署需两步确认——先披露将执行的全部操作（安装 Bun 运行时、克隆官方仓库、创建 systemd 服务），再键入版本号，后端对 `confirm` 字段做一致性校验；卸载与放行端口同样要求前端二次确认标记。**不存在任何自动触发、聊天指令触发或隐蔽执行路径。**
> - **来源固定可审计**：仅从官方渠道获取内容——Bun 官方安装脚本（`https://bun.sh/install`）与 DG-LAB 官方仓库（`dungeonlab-open/dglab-websocket-server`，浅克隆最新提交），不引入任何第三方/作者自控源；systemd 单元内容为纯静态渲染（见 `_pages_api.py` 的 `_render_relay_unit`），可在源码中逐行审计。
> - **最小权限与可逆**：部署**不修改防火墙**（端口默认仅本机可达，公网暴露由独立开关另行确认）；ufw 操作均以 `ufw status` 实际回查校验并逐条记入插件日志；卸载一键完全回滚（停服务、删单元、收回放行），源码目录保留。
> - **无隐蔽持久化**：systemd 单元仅用于运行官方中转服务本体（`Description` 注明来源），不写任何定时任务、不修改 shell 配置、不收集任何信息。
>
