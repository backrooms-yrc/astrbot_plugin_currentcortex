...
自動辨識平台並解析小紅書、B站、抖音、微博的媒體連結，回傳無浮水印圖片與影片（影片直接傳送到聊天視窗，無需點連結）。群裡有人貼連結即可自動觸發，無需指令。
...
- **B站**：標題、UP主、影片長度、畫質、封面——影片**直接傳送到聊天視窗**（見下方「影片直接傳送」）
- **抖音**：標題、作者、按讚／留言／分享、無浮水印影片**直接傳送**；圖集作品逐張傳送原圖（最多 9 張）
- **微博**：正文、作者、轉發／留言／按讚數、配圖（最多 9 張）、影片直接傳送（如有）

#### 解析增強

- **連結自動解析（無需指令）**：群裡有人貼 小紅書／B站／抖音／微博 的連結時，機器人自動解析並回覆，不必再發 `/解析`。指令訊息不會重複觸發；同一條連結在同一個群裡預設 30 分鐘內只自動解析一次（防止反覆轉發洗版），`/解析` 指令不受此限制可隨時強制重新解析；`/开关 off media` 可按群同時關閉指令與自動解析。
- **影片直接傳送**：解析到影片時，機器人直接把影片傳到聊天視窗裡，而不是丟一條點不開的防盜連直鏈。B站經 LeiZ API 由伺服器下載合併好的 MP4（畫質可在設定中下拉選擇：480P / 720P / 1080P / 4K / 自動最高，預設 1080P）；抖音傳送無浮水印影片。影片過大（超過大小上限）或下載失敗時自動改為傳送連結。暫存檔落 `/tmp/astrbot_media`，1 小時自動清理。
- **LeiZ API 解析（B站／抖音）**：已設定 `leiz_api_key` 時，B站／抖音解析優先走 LeiZ API——B站可解鎖訪客拿不到的高畫質（最高 4K，視站點帳號權益與影片來源而定），抖音拿無浮水印直鏈並支援圖集；請求不直連平台官方介面，不受 B站風控波及。LeiZ 不可用時自動回退官方路徑，功能不中斷。
...
| 設定項 | 類型 | 預設值 | 說明 |
| --- | --- | --- | --- |
| `media_auto_parse_enable` | bool | true | 【自動解析】群裡有人貼支援平台的連結時，不用下指令、自動解析並回覆 |
| `media_auto_parse_dedup_min` | int | 30 | 【自動解析】同一條連結在同一個群裡多少分鐘內只自動解析一次。0 = 每次都解析 |
| `media_video_send_enable` | bool | true | 【影片直接傳送】解析到影片時直接把影片傳到聊天視窗。傳不出去時自動改為傳連結 |
| `media_video_max_mb` | int | 100 | 【影片直接傳送】影片最大傳多大（MB）。超過就只傳連結 |
| `media_video_quality` | string | 1080P | 【影片直接傳送】B站影片畫質（下拉：480P / 720P / 1080P / 4K / 自動最高）。需設定 `leiz_api_key`；4K 檔案大，長影片可能只傳連結 |
