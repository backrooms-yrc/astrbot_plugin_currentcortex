...
DG-LAB 设备本体通过低功耗蓝牙（BLE）与手机 App 通信；远程控制时，手机 App 与中转服务器之间、本插件与中转服务器之间通过 WebSocket 传输数据。设备蓝牙协议详见 DG-LAB 开源仓库。设备具有两个独立输出通道（A 通道和 B 通道），每个通道可独立设置强度（0-200）和波形。
...
- **远程控制**：经 App 与中转服务器 WebSocket 转发，实现网络远程操控
- **安全机制**：设备端可设置强度上限，防止远程端超限操作
- **低功耗蓝牙**：采用 BLE 协议，续航持久

## 连接方式

本插件作为「控制端」连接 DG-LAB 中转服务器（官方或自建），生成二维码后由手机 App 扫码配对，配对成功后即可经服务器中转控制设备：

\`\`\`
设备 --BLE--> 手机 App --WebSocket--> 中转服务器 <--WebSocket-- 本插件 / CCDG WebUI
\`\`\`

### 连接流程

1. 在插件配置中填写中转服务器地址，或在绑定指令中直接指定
2. 在聊天中使用 \`/dglab bind [服务器地址]\` 绑定设备（地址可省略，缺省使用配置中的服务器地址）
3. 插件连接中转服务器并获得配对 ID，回执中显示二维码
4. 按你的 App 版本扫码配对（推荐 DG-LAB 4，v3 / v4 具体操作见下）
5. 配对成功后即可通过指令或 CCDG WebUI 控制

### App 版本选择

- **推荐使用 DG-LAB 4 App**：官方推荐版本，支持郊狼 2.0 / 3.0 主机，下载地址 <https://dungeon-lab.cn/>
- DG-LAB 3.0 App 仅供老用户兼容；**3.x 与 4.x 之间无法互相远程控制**

### v3 APP（DG-LAB 3.0 App）配对步骤

1. 打开 DG-LAB 3.0 App 并登录，点击**主界面上的「SOCKET」**
2. 打开郊狼 3.0 主机电源，App 会自动扫描并通过蓝牙连接设备（首次进入会弹出 WebSocket 功能介绍与注意事项，请认真阅读）
3. 在 SOCKET 页面点击**「连接服务器」**，选择**相机图标**
4. 对准插件回执中的二维码扫描，尽量让扫描框与二维码完全对齐；App 识别后会自动建立连接

> 说明：App 不支持手动输入服务器地址，只能扫码配对。中转服务器端口一般为 9999，控制端裸连根路径（如 \`ws://host:9999\`），二维码以 dungeon-lab.com/app-download.php 开头。

### v4 APP（DG-LAB 4.0 App）配对步骤

1. 下载安装 **DG-LAB 4 App**（<https://dungeon-lab.cn/>），进入软件后先在 App 内通过蓝牙连接郊狼主机
2. 依次进入 **Socket控制 → 创建Socket控制**
3. **扫描二维码**：对准插件回执中的二维码扫描（也可直接用手机相机扫，二维码为 dungeon-lab.cn/s/ 通用链接，会唤起 App）
4. App 识别后自动完成配对，回执中设备状态变为「已连接」

> 说明：新版二维码（dungeon-lab.cn/s/ 开头）**仅 DG-LAB 4 App 可识别**，3.x 无法识别。中转服务器端口一般为 9998，可带路径前缀（如 \`wss://host:9998/v4\`）。若 App 内连接了多台设备（多插槽），插件自动选择第一个真实连接的设备插槽。

### 注意事项

> 请确保设备与手机的蓝牙连接稳定，手机 App 保持运行（含后台）。二维码仅在绑定会话期间有效，超时需重新 \`/dglab bind\`。局域网内建议使用 \`ws://\`，跨公网建议使用 \`wss://\`。
...
| 通道数 | 2（A/B） |
| 强度范围 | 0-200 |
| 通信协议 | 蓝牙 BLE（设备 ↔ 手机 App）+ WebSocket（App / 插件 ↔ 中转服务器） |
| 波形频率 | 设备端 10-240；App 输入 10-1000 经换算后下发 |
| 协议参考 | DG-LAB 开源蓝牙协议（coyote/v3） |
...
将 DG-LAB 设备绑定到当前用户：插件连接中转服务器（地址可省略，缺省使用配置中的服务器地址）并生成二维码，用手机 App 扫码完成配对。绑定回执中会标注识别出的协议版本（v3 / v4）。
...
1. 确认中转服务器地址格式正确（以 \`ws://\` 或 \`wss://\` 开头），端口与路径前缀和服务端一致（v3 默认 9999、v4 默认 9998）
2. 出现「等待服务器分配 clientId 超时」：多为插件版本过旧或服务器未运行，请升级插件并确认服务器已启动
3. 出现「HTTP 404」：v4 服务器仅在根路径（或配置的 PREFIX 路径）接受连接，请检查地址路径是否一致
4. 确认手机与服务器网络连通；绑定成功后需在二维码有效期内用 App 扫码

### 设备显示"未连接"？

- 检查手机 App 是否被系统后台杀死或蓝牙断开
- 检查手机网络与中转服务器连通性
- 等待自动重连，或重新 \`/dglab bind\` 生成新二维码扫码配对

### v4 绑定后控制无效？

- 确认使用的是 DG-LAB 4 App 扫码（新版二维码旧版 App 无法识别）
- 若 App 内连接了多台设备，插件会自动选择第一个真实连接的设备插槽
- 确认设备处于"已连接"状态
